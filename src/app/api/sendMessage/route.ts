...
import { Role, Message } from '@/components/core/ChatBot';
...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const { text } = await generateText({
			...
			${messagesList.length && `Here is a complete history of our past conversation, in order: ${JSON.stringify(messagesList, null, 2)}`}
